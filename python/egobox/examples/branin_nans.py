...
The Branin function is modified to produce NaN if x1*x2 <= 0.2
This illustrates the management of hidden constraints by Egor
...
def plot_constrained_branin(res):
    ...
    x2_constraint = BRANIN_CSTR_CONST / x1_constraint
    ...
    ax.plot(
        ...
        "r-",
        linewidth=2,
        label=f"Constraint: x₁·x₂ = {BRANIN_CSTR_CONST}",
    )

    # Shade infeasible region (x1*x2 < BRANIN_CSTR_CONST)
    X1_flat = X1.ravel()
    X2_flat = X2.ravel()
    infeasible = X1_flat * X2_flat < BRANIN_CSTR_CONST
    ax.scatter(
        X1_flat[infeasible],
        X2_flat[infeasible],
        c="red",
